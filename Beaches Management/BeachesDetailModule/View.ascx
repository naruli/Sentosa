<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="View.ascx.cs" Inherits="Sentosa.Modules.BeachesDetailModule.View" %>
<script type="text/javascript">
    function GetBeachesDetail(IdDetail) {
        $.getJSON(
           "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/BeachesDetailModule/API/ModuleBeachesDetail/GetBeachesDetail?id=" + IdDetail,
           function (result) {
               var parsedTaskJSONObject = jQuery.parseJSON(result);
               $.each(parsedTaskJSONObject, function () {
                   var html = "https://maps.googleapis.com/maps/api/staticmap?center=" + this.Zone.replace(" ", "+") + "&amp;zoom=15&amp;size=320x240&amp;markers=color:red%7C" + this.Zone.replace(" ", "+");
                   ...
                   //alert(html);
                   $('section#page-title h1').html(this.Title);
                   $('section#page-title p').html('<img src="<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/portals/_default/skins/hammerflex/img/icon-map-purple.png" alt="" title="" class="">&nbsp;' + this.Zone);
                   $('section.page-article aside a.overlay-open').html('<img src="' + html + '" class="map img-responsive">');
               });
           });
    }
    function GetListBeaches(IdDetail) {
        $.getJSON(
           "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/BeachesDetailModule/API/ModuleBeachesDetail/GetListBeaches?id=" + IdDetail,
           function (result) {
               var parsedTaskJSONObject = jQuery.parseJSON(result);
               var counter = 0;
               $('#beachListCarousel').html('');
               $('#beachListCarousel').append(
               ...
               $.each(parsedTaskJSONObject, function () {
                   if (counter % 4 == 0) {
                       $('#beachListCarousel .carousel-indicators').append(
                            '<li data-target="#beachListCarousel" data-slide-to="' + counter + '" class="' + (counter == 0 ? "active" : "") + '"></li>'
                        );
                       $('#beachListCarousel .carousel-inner').append(
                   ...
                   $('#beachListCarousel .carousel-inner .item .row').append(
                       '<div class="col-xs-12 col-sm-6 col-md-3">' +
                           '<div class="box-item">' +
                               '<a href="<%=Request.Url.GetLeftPart(UriPartial.Authority)%>/Explore/Beaches/Details?Id=' + this.Id + '">' +
                                ...
                                '</a>' +
                                   '<div class="box-nav"><a href="#">' +
                                       ...
                                       (this.UrlBrochure != "" ? '<a href="' + this.UrlBrochure + '" target="_blank"><img src="<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath.ToString())%>/portals/_default/skins/hammerflex/img/icon-tag.png" alt="" title="" class=""></a>' : '') +
                                       '<a href="<%=Request.Url.GetLeftPart(UriPartial.Authority)%>/Explore/Beaches/Details?Id=' + this.Id + '">Find out more <i class="fa fa-angle-right"></i></a>' +
                                   '</div>' +
                           '</div>' +
                       '</div>'
                       );
               ...
               if (counter > 4) {
                   $('#beachListCarousel').append(
                       '<a class="left carousel-control" href="#beachListCarousel" role="button" data-slide="prev"></a>' +
                       '<a class="right carousel-control" href="#beachListCarousel" role="button" data-slide="next"></a>'
                       );
               }
               if (counter == 0) {
                   $('section#beach-list .desc').html('"Data Not Found for Beaches Listing."');
               }
           });
    }
    function GetGalleryDetail(IdDetail) {
       $.getJSON(
          "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/BeachesDetailModule/API/ModuleBeachesDetail/GetGalleryDetail?id=" + IdDetail,
       function (result) {
           var parsedTaskJSONObject = jQuery.parseJSON(result);
           var counter = 0;
           ...
           $.each(parsedTaskJSONObject, function () {
               ...
               $('div#overlay-photos #overlayCarousel .carousel-inner').append(
                    '<div class="item' + (counter == 1 ? " active" : "") + '">' +
                        ...
                        '<div class="carousel-caption">' + this.Description + '</div>' +
                    '</div><!--/.item -->'
                   );
               $('div#overlay-photos .image-lists .image-lists-inner').append(
                    '<li><a href="#" class="" data-target="#overlayCarousel" data-item="' + counter + '"><img src="' + this.Path + '" alt=""></a></li>'
                   );
           });
           if (counter == 0) {
   ...
   function GetSubMenuPageDetail(IdDetail) {
       $.getJSON(
          "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/BeachesDetailModule/API/ModuleBeachesDetail/GetSubPageDetail?id=" + IdDetail,
           function (result) {
               var parsedTaskJSONObject = jQuery.parseJSON(result);
               var counter = 0;
               $('nav.sub-nav ul').html('');
               ...
               $.each(parsedTaskJSONObject, function () {
                   ...
                   $('section.page-article article').append(
                       ...
                       (this.SafetyTitle != "" ? '<p>Safety Tips Title : ' + this.SafetyTitle + '</p>' : '') +
                       (this.SafetyInfo != "" ? '<p>Safety Tips Information : ' + this.SafetyTitle + '</p>' : '') +
                       (this.Instruction != "" ? '<p>Instruction To Get There : ' + this.Instruction + '</p>' : '') +
                       '</div>'
                       );
                   counter++;
               });
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-xs-12">
                ...
                <h2 class="title">Beach Listing</h2>
                <p class="desc"></p>
                <div id="beachListCarousel" class="carousel slide" data-ride="carousel">
