<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="View.ascx.cs" Inherits="Sentosa.Modules.DiningDetailModule.View" %>
<script type="text/javascript">
    function GetDiningDetail(IdDetail) {
        $.getJSON(
           "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/DiningDetailModule/API/ModuleDiningDetail/GetDiningDetail?id=" + IdDetail,
           function (result) {
       ...
       function GetGalleryDetail(IdDetail) {
           $.getJSON(
              "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/DiningDetailModule/API/ModuleDiningDetail/GetGalleryDetail?id=" + IdDetail,
           function (result) {
               var parsedTaskJSONObject = jQuery.parseJSON(result);
               var counter = 0;
   ...
   function GetSubMenuPageDetail(IdDetail) {
       $.getJSON(
          "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/DiningDetailModule/API/ModuleDiningDetail/GetSubPageDetail?id=" + IdDetail,
           function (result) {
               var parsedTaskJSONObject = jQuery.parseJSON(result);
               var counter = 0;
               $('nav.sub-nav ul').html('');
               ...
               $.each(parsedTaskJSONObject, function () {
                   ...
                   $('section.page-article article').append(
                       ...
                       (this.Promotion != "" ? '<p>Opening Hour : ' + this.Promotion + '</p>' : '') +
                       '</div>'
                       );
                   counter++;
               });
