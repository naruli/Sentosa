<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="View.ascx.cs" Inherits="Sentosa.Modules.EventModule.View" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        listEvents(idList, list);
        searchEvents(searchValue, sortBy, list, offset, limit);
        //console.log('Parameter : ' + searchValue + ', ' + sortBy + ', ' + list);
    });

    function searchEvents(paramSearch, paramOrder, paramList, paramOffset, paramLimit) {
        $.getJSON(
            "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/EventModule/API/ModuleEvent/SearchEvents?searchValue=" + paramSearch + "&order=" + paramOrder + "&list=" + paramList + "&offset=" + paramOffset + "&limit=" + paramLimit,
           function (result) {
               ...
               $.each(parsedTaskJSONObject, function () {
                   ...
                   $('#resultTable').append(
                       '<div class="col-xs-12 col-sm-6 col-md-3">' +
                            '<div class="box-item">' +
                                '<a href="Events/Details?Id=' + this.Id + '">' +
                                 ...
                                 '<div class="box-nav">' +
                                    ...
                                    '<a href="Events/Details?Id=' + this.Id + '">Find out more <i class="fa fa-angle-right"></i></a>' +
                                 '</div><!--/ .box-nav -->' +
                            '</div><!--/ .box-item -->' +
                        '</div><!--/ .col-xs-12 -->'
                       );
               ...
               if (counter > 0) {
                   ...
                   var page = Math.floor(total / parseInt(paramLimit)) - (total % parseInt(paramLimit) == 0 ? 1 : 0);
                   if (page > 0) {
                       if (paramOffset > 1) {
                           $('#nav-paging').append(
                                '<li><a href="#">Prev</a></li>'
       ...
       function listEvents(Id, paramList) {
           $.getJSON(
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-md-3">
                <div class="form-search">
                    <input type="search" name="keywords" id="keywords" class="form-control" onkeyup="search(event);" placeholder="Search within Events">
                </div>
            </div><!--/ .col-xs-12 -->
        </div><!--/ .row -->
    </div><!--/ .container -->
