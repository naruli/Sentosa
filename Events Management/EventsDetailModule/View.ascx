<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="View.ascx.cs" Inherits="Sentosa.Modules.EventsDetailModule.View" %>



<script type="text/javascript">
    function GetEventsDetail(IdDetail) {
        $.getJSON(
           "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/EventsDetailModule/API/ModuleEventsDetail/GetEventsDetail?id=" + IdDetail,
           function (result) {
       ...
       function GetGalleryDetail(IdDetail) {
           $.getJSON(
              "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/EventsDetailModule/API/ModuleEventsDetail/GetGalleryDetail?id=" + IdDetail,
           function (result) {
               var parsedTaskJSONObject = jQuery.parseJSON(result);
               var counter = 0;
   ...
   function GetSubMenuPageDetail(IdDetail) {
       $.getJSON(
          "<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath)%>/DesktopModules/EventsDetailModule/API/ModuleEventsDetail/GetSubPageDetail?id=" + IdDetail,
           function (result) {
               var parsedTaskJSONObject = jQuery.parseJSON(result);
               var counter = 0;
               $('nav.sub-nav ul').html('');
               ...
               $.each(parsedTaskJSONObject, function () {
                   ...
                   $('section.page-article article').append(
                       ...
                       (this.Instruction != "" ? '<p>Instructions of getting there : ' + this.Instruction + '</p>' : '') +
                       '</div>'
                       );
                   counter++;
               });
