        ...
        $.getJSON(
           ...
           function (result) {
               ...
               $.each(parsedTaskJSONObject, function () {
                   if (this.TypeHappening == "P_EVENT") {
                       counterEvent++;
                       $('#Events').append(
                           '<tr>' +
                           '<td style="width:10%;text-align:center;">' +
                           '<input type="checkbox" name="event' + counterEvent + '" id="event' + this.TabID + '" value="' + this.TabID + '"/>' +
                           '</td>' +
                           '<td colpan="2">' +
                           this.TabName +
                           '</td>' +
                       ...
                       $('#Promotions').append(
                           '<tr>' +
                           '<td style="width:10%;text-align:center;">' +
                           '<input type="checkbox" name="promotion' + counterPromotion + '" id="promotion' + this.TabID + '" value="' + this.TabID + '"/>' +
                           '</td>' +
                           '<td colpan="2">' +
                           this.TabName +
                           '</td>' +
               ...
               setCheckbox('<%=(Request.ApplicationPath.Equals("/") ? "" : Request.ApplicationPath.ToString())%>/DesktopModules/HappeningModule/API/ModuleHappening/GetHappening?TabId=<%=this.TabId%>');
           });
    }
